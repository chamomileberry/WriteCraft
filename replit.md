...
- **Analytics**: PostHog integration for product analytics and user behavior tracking; tracks page views (authenticated and anonymous), user identification with subscription tier, content generation events, AI assistant usage, and key user actions; privacy-focused with input masking and data-private selectors; readiness-aware implementation prevents dropped events; free tier provides 1M events/month, unlimited users, and 1-year retention.
