...
import { 
  users, characters, plots, prompts, locations, settings, items,
  organizations, creatures, species, cultures, documents, foods,
  languages, religions, technologies, weapons, professions, ranks,
  conditions, savedItems, names, themes, moods, conflicts,
  guides, guideCategories, guideReferences, projects, projectSections,
  projectLinks, folders, notes, notebooks, importJobs,
  plants, descriptions, ethnicities, drinks, armor, accessories,
  clothing, materials, settlements, societies, factions, militaryUnits,
  myths, legends, events, spells, resources, buildings, animals,
  transportation, naturalLaws, traditions, rituals, familyTrees,
  familyTreeMembers, familyTreeRelationships, timelines, timelineEvents,
...
`);
  ...
  try {
    ...
    if (timelineRelData.length > 0) {
      sqlStatements.push(generateInsert('timeline_relationships', timelineRelData));
      console.log(`   ✓ Exported ${timelineRelData.length} timeline relationships`);
    }
    ...
    if (familyMemberData.length > 0) {
      sqlStatements.push(generateInsert('family_tree_members', familyMemberData));
      console.log(`   ✓ Exported ${familyMemberData.length} family tree members`);
    }

    const familyRelData = await db.select().from(familyTreeRelationships);
    if (familyRelData.length > 0) {
      sqlStatements.push(generateInsert('family_tree_relationships', familyRelData));
