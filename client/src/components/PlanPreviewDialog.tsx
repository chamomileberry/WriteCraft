...
export function PlanPreviewDialog({
  ...
  const previewMutation = useMutation({
    mutationFn: async (variables: {
      ...
      const response = (await apiRequest(
        "POST",
        "/api/stripe/preview-subscription-change",
        {
          tier: variables.tier,
  ...
  useEffect(() => {
    if (open && hasActiveSubscription) {
      setPreview(null);
      setPreviewError(null);
      const validCode = discountValidation?.valid
  ...
  const planPrice =
    billingCycle === "annual" ? tierInfo.annualPrice : tierInfo.price;
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                {!isValidatingDiscount && discountValidation?.valid && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <CheckCircle2
                      ...
                      data-testid="icon-valid-discount"
                    />
                  </div>
                )}
                {!isValidatingDiscount &&
                  ...
                  !discountValidation.valid && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      <XCircle
                        ...
                        data-testid="icon-invalid-discount"
                      />
                    </div>
                  )}
              </div>
              {discountValidation?.valid && (
                <p
                  ...
                  data-testid="text-discount-success"
                >
            ...
            <div className="space-y-3">
              ...
              {preview.lineItems.map((item, index) => (
                ...
                >
                  <span
                    className={item.isProration ? "text-muted-foreground" : ""}
                  >
                    {item.description}
                  </span>
                  <span
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border p-4 space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="secondary"
                  data-testid="badge-billing-cycle-new"
                >
                  {billingCycle === "annual" ? "Annual" : "Monthly"}
                </Badge>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                {!isValidatingDiscount && discountValidation?.valid && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <CheckCircle2
                      className="h-4 w-4 text-green-600"
                      data-testid="icon-valid-discount-new"
                    />
                  </div>
                )}
                {!isValidatingDiscount &&
                  ...
                  !discountValidation.valid && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      <XCircle
                        className="h-4 w-4 text-destructive"
                        data-testid="icon-invalid-discount-new"
                      />
                    </div>
                  )}
              </div>
              {discountValidation?.valid && (
                <p
                  className="text-sm text-green-600"
                  data-testid="text-discount-success-new"
                >
                  {discountValidation.type === "percentage"
              ...
              {discountValidation && !discountValidation.valid && (
                <p
                  className="text-sm text-destructive"
                  data-testid="text-discount-error-new"
                >
                  {discountValidation.message}
                </p>
              )}
