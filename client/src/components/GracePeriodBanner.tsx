...
export function GracePeriodBanner() {
  ...
  if (gracePeriodExpired) {
    return (
      <div
        className="border-b border-destructive bg-destructive/10"
        data-testid="banner-grace-expired"
      >
            ...
            <AlertDescription className="flex items-center justify-between gap-4 ml-2">
              <div className="flex-1">
                ...
                <p className="text-sm text-destructive/90 mt-1">
                  Your grace period has ended. Please upgrade your plan or
                  reduce your usage to continue using WriteCraft.
                </p>
                {warnings.length > 0 && (
  ...
  if (inGracePeriod && hasExceededLimits) {
    return (
      <div
        className="border-b border-orange-500 bg-orange-50 dark:bg-orange-950/30"
        data-testid="banner-grace-active"
      >
        <div className="container mx-auto">
          <Alert className="border-0 bg-transparent rounded-none">
            ...
            <AlertDescription className="flex items-center justify-between gap-4 ml-2">
              <div className="flex-1">
                <p className="font-semibold text-orange-800 dark:text-orange-300">
                  Grace Period Active: {gracePeriodDaysRemaining}{" "}
                  {gracePeriodDaysRemaining === 1 ? "day" : "days"} remaining
                </p>
                <p className="text-sm text-orange-700 dark:text-orange-400 mt-1">
                  You've exceeded your plan limits. Upgrade or reduce usage
                  before the grace period ends to avoid service interruption.
                </p>
                {warnings.length > 0 && (
