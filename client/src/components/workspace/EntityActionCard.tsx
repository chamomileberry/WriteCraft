...
const entityConfig: Record<string, { icon: LucideIcon; label: string; color: string }> = {
  character: {
    icon: User,
    label: 'Character',
    color: 'text-blue-600'
...
export default function EntityActionCard({
  ...
  const getPreview = () => {
    if (entity.type === 'character') {
      ...
      return entity.details.description?.split('.')[0] || 'Location details extracted';
    } else {
      return entity.details.description?.split('.')[0] || 'Plot point details extracted';
    }
  };

  return (
    <div className="flex items-start gap-3 p-3 bg-background border rounded-lg hover-elevate">
      <div className={`flex-shrink-0 w-10 h-10 rounded-full bg-muted flex items-center justify-center ${config.color}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-2">
          <Tooltip>
            ...
            <TooltipContent>
              <p className="text-xs">Create a new {config.label.toLowerCase()} with extracted details</p>
            </TooltipContent>
          </Tooltip>
          ...
          {existingEntity && onUpdateExisting && (
            <Tooltip>
              ...
              <TooltipContent>
                <p className="text-xs">Update the existing {config.label.toLowerCase()}</p>
              </TooltipContent>
            </Tooltip>
          )}
        </div>
