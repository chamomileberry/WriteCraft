...
export default function WritingAssistantPanel({
  ...
  const { user } = useAuth();
  const { hasPremiumAccess, extendedThinkingRemaining, premiumQuota } =
    useSubscription();
  ...
  const { data: characters } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/characters?notebookId=${activeNotebookId}`,
        {
          credentials: "include",
        },
      );
  ...
  const suggestedPrompts: SuggestedPrompt[] = [
    {
      ...
      label: "Analyze",
      prompt:
        "Analyze the current text for readability, style, and provide improvement suggestions",
    ...
    },
    {
      ...
      label: "Proofread",
      prompt:
        "Proofread the current text for grammar, spelling, and style issues",
        ...
        if (threadIdFromUrl) {
          ...
          try {
            const threadResponse = await fetch(
              `/api/conversation-threads/${threadIdFromUrl}`,
              {
                credentials: "include",
              },
            );
            ...
            if (threadResponse.ok) {
              ...
              const messagesResponse = await fetch(
                `/api/chat-messages?threadId=${threadIdFromUrl}`,
                {
                  credentials: "include",
                },
              );
              ...
              if (messagesResponse.ok) {
                ...
                const formattedMessages: Message[] = chatMessages.map(
                  (msg: any) => ({
                    id: msg.id,
                    type: msg.type,
                    content: msg.content,
          ...
          if (response.ok) {
            ...
            if (chatMessages.length > 0 && chatMessages[0].threadId) {
              ...
              try {
                const threadResponse = await fetch(
                  `/api/conversation-threads/${chatMessages[0].threadId}`,
                  {
                    credentials: "include",
                  },
                );
  ...
  const chatMutation = useMutation({
    mutationFn: async (message: string) => {
      ...
      const hasEditorContent =
        editorContext.content && editorContext.content.length > 10;
      ...
      const response = await fetch("/api/writing-assistant/chat", {
        ...
        body: JSON.stringify({
          ...
          projectId:
            editorContext.type === "manuscript"
              ? editorContext.entityId
              : undefined,
          guideId:
            editorContext.type === "guide" ? editorContext.entityId : undefined,
    ...
    onError: (error: any) => {
      ...
      if (error?.response?.status === 429) {
        errorMessage =
          "You've reached your usage limit. Please wait a moment and try again.";
      } else if (
  ...
  const detectEntities = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error) {
          toast({
            ...
            description:
              "Could not extract character/location details from the conversation.",
            variant: "destructive",
          });
        }
      } else {
      ...
      toast({
        ...
        description:
          "Could not analyze the conversation for entities. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const extractTextSuggestions = (content: string) => {
    ...
    const correctedTextRegex =
      /(?:corrected|improved|rephrased|better).*?:\s*"([^"]+)"/gi;
  ...
  const applyTextChange = (
    text: string,
    type: "replace" | "insert" = "replace",
  ) => {
    try {
      const success = executeEditorAction(
        type === "replace" ? "replaceSelection" : "insertAtCursor",
        text,
      );
      if (success) {
        toast({
  ...
  const addMessage = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === newMessage.id ? { ...msg, id: savedMessage.id } : msg,
          ),
        );
  ...
  const handleSuggestedPrompt = (prompt: SuggestedPrompt) => {
    ...
    if (
      !editorContent.hasEditorContent &&
      (prompt.id === "analyze" ||
        prompt.id === "proofread" ||
        prompt.id === "questions")
    ) {
  ...
  const handleUpdateEntity = async (entity: DetectedEntity) => {
    ...
    try {
      if (entity.type === "character") {
        ...
        if (entity.details.age && !existingChar.age)
          updates.age = parseInt(entity.details.age);
        if (entity.details.species && !existingChar.species)
          updates.species = entity.details.species;
        if (entity.details.occupation && !existingChar.occupation)
          updates.occupation = entity.details.occupation;
        ...
        if (entity.details.personality) {
          ...
          if (personalityArray.length > 0) {
            const existingTraits = Array.isArray(existingChar.personality)
              ? existingChar.personality
              : [];
            const newTraits = personalityArray.filter(
        ...
        if (entity.details.physicalDescription) {
          const updated = appendOrSet(
            existingChar.physicalDescription,
            entity.details.physicalDescription,
          );
        ...
        if (entity.details.backstory) {
          const updated = appendOrSet(
            existingChar.backstory,
            entity.details.backstory,
          );
        ...
        if (entity.details.motivation) {
          const updated = appendOrSet(
            existingChar.motivation,
            entity.details.motivation,
          );
        ...
        if (entity.details.strength) {
          const updated = appendOrSet(
            existingChar.strength,
            entity.details.strength,
          );
        ...
        if (entity.details.dislikes) {
          const updated = appendOrSet(
            existingChar.dislikes,
            entity.details.dislikes,
          );
        ...
        if (Object.keys(updates).length > 0) {
          const response = await fetch(
            `/api/characters/${existingChar.id}?notebookId=${activeNotebookId}`,
            {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              credentials: "include",
              ...
              description: (
                <div className="space-y-2">
                  <p className="font-medium">
                    Added/updated the following details:
                  </p>
  ...
  const MessageWithApplyButtons = React.memo(
    ({
      message,
      isLastAssistant,
    }: {
      ...
      const editorContext = getEditorContext();
      const hasEditorAvailable =
        editorContext.type !== null && editorContext.entityId !== null;

      // Show contextual prompts only on the last assistant message
      const showContextualPrompts =
        isLastAssistant &&
        message.type === "assistant" &&
        contextAnalysis &&
        contextAnalysis.topics.length > 0;
      ...
      return (
        ...
        >
          ...
          <div
            className={`max-w-[85%] ${message.type === "user" ? "order-first" : ""}`}
          >
            <div
                  ...
                  {suggestions.map((suggestion, index) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          applyTextChange(suggestion.text, suggestion.type)
                        }
                        className="h-8 px-3"
  ...
  return (
    <div
      className={`h-full flex flex-col bg-background overflow-hidden ${className}`}
      data-testid={`writing-assistant-panel-${panelId}`}
    >
          ...
          {messages.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-xs mt-2">
                I have full context of your notebook and projects.
              </p>
              <p className="text-xs mt-1">
                Try the suggested prompts below to get started.
              </p>
            </div>
          )}
            ...
            <div className="space-y-2">
              {detectedEntities.map((entity, index) => (
                <EntityActionCard
                  ...
                  existingEntity={
                    entity.type === "character"
                      ? findExistingCharacter(entity.name)
                      : undefined
                  }
                />
              ))}
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-2">
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="text-xs">
                    Uses advanced AI for deeper reasoning and analysis
                  </p>
                </TooltipContent>
              </Tooltip>
            ...
            <span className="text-xs text-muted-foreground">
              {extendedThinkingRemaining}/
              {premiumQuota?.extendedThinking.limit || 0} remaining
            </span>
          </div>
        )}
            ...
            if (selectedEntity.type === "character") {
              ...
              if (response.ok) {
                ...
                setDetectedEntities((prev) =>
                  prev.filter((e) => e !== selectedEntity),
                );
                // Invalidate characters query
