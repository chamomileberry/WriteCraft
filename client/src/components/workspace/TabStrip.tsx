...
export function TabStrip({
  ...
  const tabs = allTabs.filter(
    (tab) => tab.type !== "manuscript" && tab.type !== "manuscriptOutline",
  );
  ...
  const handleTabDragStart = (
    ...
    const tabIndex = Array.from(
      tabElement.parentElement?.children || [],
    ).indexOf(tabElement);
  ...
  const handleDetachTab = (panelId: string) => {
    const rect = document
      .querySelector(`[data-tab-id="${panelId}"]`)
        ...
        <div
          ...
          onClick={() =>
            !isEditingProject && !activeTab
              ? handleStartProjectEdit()
              : !isEditingProject && setActiveTab("", "main")
          }
      ...
      {tabs.map((tab, index) => (
        ...
        >
          ...
          <Button
            ...
            title={
              tab.type === "quickNote" && !isInManuscriptEditor()
                ? "Pop out to floating window"
                : tab.type === "writingAssistant"
                  ? "Minimize"
                  : "Pin to floating window"
            }
          >
            {(tab.type === "quickNote" && !isInManuscriptEditor()) ||
            tab.type === "writingAssistant" ? (
