...
export default function WritingAssistantTrigger() {
  ...
  const handleClick = () => {
    ...
    const safeX = Math.max(
      20,
      Math.min(window.innerWidth - panelWidth - 20, window.innerWidth - 450),
    );
    const safeY = Math.max(
      20,
      Math.min(window.innerHeight - panelHeight - 20, 100),
    );
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            <Sparkles
              className={`w-6 h-6 text-primary-foreground transition-transform duration-300 ${isHovered ? "scale-110" : ""}`}
            />
          </Button>
        </TooltipTrigger>
        <TooltipContent
