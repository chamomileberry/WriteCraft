...
import {
  ...
  type AISuggestion,
} from "@/lib/ai-suggestions-plugin";
...
export default function AIBubbleMenu({ editor }: AIBubbleMenuProps) {
  ...
  const [askSuggestionPosition, setAskSuggestionPosition] = useState({
    from: 0,
    to: 0,
  });
  ...
  useEffect(() => {
    ...
    if (pluginState && pluginState.suggestions.length > 0) {
      const activeSuggestion = pluginState.suggestions.find(
  ...
  const hasActiveSuggestion = () => {
    ...
    return (
      pluginState &&
      pluginState.suggestions.some((s: AISuggestion) => s.status === "pending")
    );
  };
  ...
  const handleAcceptSuggestion = () => {
    ...
    const updatedSuggestions = pluginState.suggestions.filter(
      (s: AISuggestion) => s.id !== activeSuggestion.id,
    );
  ...
  const getActiveSuggestion = () => {
    ...
    return pluginState.suggestions.find(
      (s: AISuggestion) => s.status === "pending",
    );
  };
  ...
  return (
    <>
      ...
      >
        <Card
          className="flex items-center gap-1 p-1 shadow-lg border"
          style={{ zIndex: 99999 }}
        >
          <Tooltip>
          ...
          <Card className="max-w-md p-4 shadow-xl border">
            <div className="space-y-3">
              ...
              <div className="inline-flex items-center gap-1 text-xs font-semibold text-purple-700 bg-purple-100 px-2 py-1 rounded">
                ...
                {activeSuggestion.type.charAt(0).toUpperCase() +
                  activeSuggestion.type.slice(1)}{" "}
                Suggestion
              </div>
