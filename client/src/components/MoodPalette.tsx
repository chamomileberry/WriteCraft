...
export default function MoodPalette() {
  ...
  const { notebooks, setNotebooks, setActiveNotebook, activeNotebookId } =
    useNotebookStore();
      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Click generate to create a mood palette with evocative sensory
            details.
          </p>
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                {generatedMood.sensoryDetails.map(
                  (detail: string, index: number) => (
                    <div key={index} className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-primary rounded-full mt-2 flex-shrink-0" />
                      <span data-testid={`text-sensory-detail-${index}`}>
                        {detail}
                      </span>
                    </div>
                  ),
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {generatedMood.colorAssociations.map(
            ...
            <div>
              ...
              <div className="space-y-2">
                {generatedMood.weatherElements.map(
                  (element: string, index: number) => (
                    <div key={index} className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0" />
                      <span data-testid={`text-weather-element-${index}`}>
                        {element}
                      </span>
                    </div>
                  ),
            ...
            <div>
              ...
              <div className="space-y-2">
                {generatedMood.lightingEffects.map(
                  (effect: string, index: number) => (
                    <div key={index} className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-yellow-500 rounded-full mt-2 flex-shrink-0" />
                      <span data-testid={`text-lighting-effect-${index}`}>
                        {effect}
                      </span>
                    </div>
                  ),
            ...
            <div>
              ...
              <div className="space-y-2">
                {generatedMood.soundscape.map(
                  (sound: string, index: number) => (
                    <div key={index} className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0" />
                      <span data-testid={`text-soundscape-${index}`}>
                        {sound}
                      </span>
                    </div>
                  ),
