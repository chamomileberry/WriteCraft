...
interface WorkspaceState {
  ...
  // Editor context for AI Writing Assistant
  updateEditorContext: (
    context: Partial<WorkspaceState["editorContext"]>,
  ) => void;
    ...
    (set, get) => ({
      ...
      addPanel: (panel: PanelDescriptor) => {
        set((state) => {
          ...
          const safeRegions = state.currentLayout.regions || {
            main: [],
            split: [],
            docked: [],
            floating: [],
          ...
          if (existingPanel) {
            ...
            get().setActiveTab(
              existingPanel.id,
              existingPanel.regionId === "split" ? "split" : "main",
            );
            return state;
          }
          ...
          const newPanel: PanelDescriptor = {
            ...
            regionId:
              panel.mode === "floating"
                ...
                : panel.mode === "docked"
                  ? "docked"
                  : "main", // Use appropriate region for panel mode
            tabIndex:
              panel.mode === "floating" || panel.mode === "docked"
                ? undefined
                : safeMainRegion.length,
          ...
          const updatedRegions = {
            ...safeRegions,
            main:
              newPanel.mode === "floating" || newPanel.mode === "docked"
                ? safeMainRegion
                : [...safeMainRegion, newPanel.id],
            docked:
              newPanel.mode === "docked"
                ? [...safeDockedRegion, newPanel.id]
                : safeDockedRegion,
          };
          ...
          return {
            currentLayout: {
              ...
              activeTabId:
                newPanel.mode === "tabbed"
                  ? newPanel.id
                  : state.currentLayout.activeTabId,
            },
          };
        });
      },
      ...
      removePanel: (panelId: string) => {
        set((state) => {
          const panel = state.currentLayout.panels.find(
            (p) => p.id === panelId,
          );
          ...
          // Remove from regions
          const updatedRegions = {
            main: state.currentLayout.regions.main.filter(
              (id) => id !== panelId,
            ),
            split: state.currentLayout.regions.split.filter(
              (id) => id !== panelId,
            ),
            docked: (state.currentLayout.regions.docked || []).filter(
              (id) => id !== panelId,
            ),
            floating: (state.currentLayout.regions.floating || []).filter(
              (id) => id !== panelId,
            ),
          };
          ...
          if (newActiveTabId === panelId && panel.mode === "tabbed") {
            const regionPanels =
              panel.regionId === "split"
                ? updatedRegions.split
                : updatedRegions.main;
            newActiveTabId =
              regionPanels.length > 0 ? regionPanels[0] : undefined;
          }
      ...
      focusPanel: (panelId: string) => {
        ...
        if (panel) {
          ...
          if (panel.mode === "tabbed") {
            get().setActiveTab(
              panelId,
              panel.regionId === "split" ? "split" : "main",
            );
          }
      ...
      isPanelOpen: (type: string, entityId?: string) => {
        ...
        return panels.some(
          (p) => p.type === type && (!entityId || p.entityId === entityId),
        );
      },
      ...
      getTabsInRegion: (regionId: "main" | "split") => {
        ...
        return panelIds
          .map((id) => state.currentLayout.panels.find((p) => p.id === id)!)
          .filter(Boolean);
      },

      getFloatingPanels: () => {
        const state = get();
        return state.currentLayout.panels.filter(
          (p) => p.mode === "floating" && !p.minimized,
        );
      },
      ...
      getActiveTab: (regionId: "main" | "split" = "main") => {
        ...
        const activePanel = state.currentLayout.panels.find(
          (p) => p.id === state.currentLayout.activeTabId,
        );
      ...
      toggleQuickNote: () => {
        const state = get();
        const quickNote = state.currentLayout.panels.find(
          (p) => p.type === "quickNote",
        );
      ...
      openQuickNote: (noteId?: string, savedNoteData?: any) => {
        const state = get();
        const existingQuickNote = state.currentLayout.panels.find(
          (p) => p.type === "quickNote",
        );
        ...
        if (!existingQuickNote) {
          ...
          const panelWidth = isMobile
            ? Math.min(window.innerWidth - 20, 300)
            : 300;
          const panelHeight = isMobile
            ? Math.min(window.innerHeight - 100, 400)
            : 400;
          const xPosition = isMobile
            ? 10
            : Math.max(10, window.innerWidth - 350);
      ...
      closeQuickNote: () => {
        const state = get();
        const quickNotePanel = state.currentLayout.panels.find(
          (p) => p.type === "quickNote",
        );
      ...
      isInManuscriptEditor: () => {
        ...
        return (
          window.location.pathname.includes("/manuscripts/") &&
      ...
      getDockedPanels: (slot: string) => {
        const state = get();
        return state.currentLayout.panels.filter(
          (p) => p.mode === "docked" && p.dockSlot === slot,
        );
      },
