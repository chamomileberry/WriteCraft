...
export default function TimelineViewPage() {
  ...
  const [activeTab, setActiveTab] = useState<"list" | "canvas" | "gantt">(
    "list",
  );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest(
        "GET",
        `/api/timelines/${id}?notebookId=${notebookId}`,
      );
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(val) =>
          setActiveTab(val as "list" | "canvas" | "gantt")
        }
        ...
        {isMobile && (
          <Alert className="mx-4 mt-4 bg-primary/5 border-primary/20">
            ...
            <AlertDescription className="text-sm">
              Canvas and Gantt views are optimized for desktop. Use List view on
