  ...
  const uploadMutation = useMutation({
    mutationFn: async (files: File[]) => {
      ...
      if (activeNotebook) {
        ...
        console.log(
          `[Import] Uploading ${files.length} file(s) to notebook: ${activeNotebook.id} (${activeNotebook.name})`,
        );
      } else {
        console.warn(
          "[Import] No active notebook selected - import will use default",
        );
      }
    ...
    onSuccess: () => {
      ...
      if (activeNotebook) {
        queryClient.invalidateQueries({
          queryKey: ["/api/saved-items/notebook", activeNotebook.id],
        });
      }
      ...
      toast({
        ...
        description:
          "Your content is being imported. This may take a few minutes.",
      });
    },
    onError: (error: Error) => {
      toast({
    ...
    if (files.length > 0) {
      ...
      if (invalidFiles.length > 0) {
        toast({
          ...
          description:
            "Please select ZIP (World Anvil) or Campfire document files (HTML, RTF, DOCX, PDF).",
          variant: "destructive",
        });
        return;
      }
      ...
      <div className="container mx-auto p-6 max-w-6xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Import your existing worldbuilding content from World Anvil (ZIP) or
            Campfire (HTML, RTF, DOCX, PDF). Select one or more files to import.
          </p>
          {!activeNotebook ? (
            <Alert className="mt-4" variant="destructive">
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                Please select or create a notebook first. Imports need a
        ...
        <div className="grid gap-6 mb-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select ZIP file(s) from World Anvil or Campfire document files
                (HTML, RTF, DOCX, PDF)
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                  <li>
                    Log in to your World Anvil account (Guild membership
                    required)
                  </li>
              ...
              <div>
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm text-muted-foreground">
                  ...
                  <li>
                    Select all exported document files and upload them together
                  </li>
                </ol>
              </div>
            </CardContent>
          ...
          <CardContent>
            {isLoadingHistory ? (
                          ...
                          {job.status === "completed" && job.results && (
                            <span>
                              ...
                              {job.results.failed.length > 0 &&
                                `, ${job.results.failed.length} failed`}
                              {job.results.skipped.length > 0 &&
                                `, ${job.results.skipped.length} skipped`}
                            </span>
                          )}
                                    ...
                                    <ul className="text-sm text-muted-foreground list-disc list-inside space-y-1">
                                      ...
                                      {job.results.skipped.length > 10 && (
                                        <li className="text-xs">
                                          ...and{" "}
                                          {job.results.skipped.length - 10} more
                                        </li>
                                      )}
                                    </ul>
                                  </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  No world-building import history yet. Upload your export to
                  get started.
                  <br />
                  Supported file types: ZIP (World Anvil), HTML, RTF, DOCX, PDF
                  (Campfire)
                </p>
              </div>
            )}
