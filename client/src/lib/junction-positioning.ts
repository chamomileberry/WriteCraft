...
export function calculateJunctionPosition(
  ...
  const parent1Height = parent1Node.measured?.height || parent1Node.height || 80;
  const parent2Height = parent2Node.measured?.height || parent2Node.height || 80;
  ...
  if (import.meta.env.DEV) {
    validateJunctionPosition(parent1Node, parent2Node, { x: junctionX, y: junctionY }, areParentsAligned);
  }
...
function validateJunctionPosition(
  ...
  if (junctionPos.x < Math.min(leftHandleX, rightHandleX) || 
  ...
  if (areParentsAligned) {
    ...
    if (Math.abs(junctionPos.y - avgCenterY) > tolerance) {
      console.warn(
        ...
        { junctionY: junctionPos.y, avgCenterY, parent1CenterY, parent2CenterY }
      );
    }
  }
}
