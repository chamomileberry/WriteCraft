...
describe("Authentication & Authorization Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const notebook1 = await createTestNotebook(user1Id, {
      name: "Alice's Notebook",
    });
    const notebook2 = await createTestNotebook(user2Id, {
      name: "Bob's Notebook",
    });
    ...
    const char1 = await createTestCharacter(user1Id, user1NotebookId, {
      givenName: "Alice's Character",
    });
    const char2 = await createTestCharacter(user2Id, user2NotebookId, {
      givenName: "Bob's Character",
    });
  ...
  describe("401 Unauthenticated Tests", () => {
    ...
    it("should return 401 for unauthenticated GET /api/notebooks request", async () => {
      const response = await request(app).get("/api/notebooks").expect(401);
