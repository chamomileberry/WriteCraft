...
async function migrateExistingUsers() {
  console.log('🚀 Starting migration: Create subscriptions for existing users...\n');
  ...
  try {
    ...
    const usersWithoutSubscription = await db
      ...
      .from(users)
      .where(
        sql`${users.id} NOT IN (SELECT user_id FROM user_subscriptions)`
      );
    ...
    console.log(`✅ Successfully created ${created.length} free-tier subscriptions\n`);
    ...
    await db
      ...
      .where(
        sql`${users.id} IN (${sql.join(usersWithoutSubscription.map(u => sql`${u.id}`), sql`, `)})`
      );
