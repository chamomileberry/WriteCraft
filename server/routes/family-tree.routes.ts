  ...
  try {
    ...
    if (!notebookId) {
      ...
      for (const notebook of notebooks) {
        const familyTrees = await storage.getUserFamilyTrees(
          userId,
          notebook.id,
        );
...
router.put("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (req.body.description !== undefined)
      familyTreeData.description = req.body.description;
    if (req.body.layoutMode !== undefined)
      familyTreeData.layoutMode = req.body.layoutMode;
    if (req.body.zoom !== undefined) familyTreeData.zoom = req.body.zoom;

    const validatedUpdates = insertFamilyTreeSchema
      .partial()
      .parse(familyTreeData);
    const updatedFamilyTree = await storage.updateFamilyTree(
      req.params.id,
      userId,
      validatedUpdates,
    );
...
router.post("/:treeId/members", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const tree = await storage.getFamilyTree(
      treeId,
      userId,
      req.body.notebookId || req.query.notebookId,
    );
    if (!tree) {
      console.warn(
        `[Security] Unauthorized tree access attempt - userId: ${userId}, treeId: ${treeId}`,
      );
      return res.status(404).json({ error: "Tree not found" });
    }

    const validatedMember = insertFamilyTreeMemberSchema.parse({
      ...req.body,
      treeId,
    });
    ...
    if (error instanceof Error && error.message.includes("Unauthorized")) {
      console.warn(
        `[Security] Unauthorized operation - userId: ${req.user?.claims?.sub}`,
      );
      return res.status(404).json({ error: "Not found" });
    }
...
router.post(
  "/:treeId/members/add-related",
  writeRateLimiter,
  async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const treeId = req.params.treeId;
      const { relativeMemberId, relationshipType, characterId, inlineName } =
        req.body;
      ...
      if (!relativeMemberId || !relationshipType) {
        return res
          .status(400)
          .json({
            error: "relativeMemberId and relationshipType are required",
          });
      }
      ...
      if (!characterId && !inlineName) {
        return res
          .status(400)
          .json({ error: "Either characterId or inlineName must be provided" });
      }
      ...
      if (!validRelationshipTypes.includes(relationshipType)) {
        return res
          ...
          .json({
            error:
              "Invalid relationship type. Must be one of: parent, spouse, child, sibling, adoption, stepParent, grandparent, cousin",
          });
      }

      // Get the relative member to calculate position
      const members = await storage.getFamilyTreeMembers(treeId, userId);
      const relativeMember = members.find(
        (m: any) => m.id === relativeMemberId,
      );
      ...
      const existingRelationships = await storage.getFamilyTreeRelationships(
        treeId,
        userId,
      );
      const existingParents = existingRelationships.filter(
        (rel: any) =>
          rel.toMemberId === relativeMemberId &&
          (rel.relationshipType === "parent" ||
            rel.relationshipType === "adoption" ||
            rel.relationshipType === "stepParent"),
      );
        ...
        case "parent":
          ...
          if (existingParents.length === 0) {
            ...
            const existingParentIds = existingParents.map(
              (rel: any) => rel.fromMemberId,
            );
            const parentMembers = members.filter((m: any) =>
            ...
            if (parentMembers.length > 0) {
              ...
              const rightmostParent = parentMembers.reduce(
                (rightmost, current) => {
                  return (current.positionX || 0) > (rightmost.positionX || 0)
                    ? current
                    : rightmost;
                },
                parentMembers[0],
      ...
      const validatedRelationship =
        insertFamilyTreeRelationshipSchema.parse(relationshipData);
      const savedRelationship = await storage.createFamilyTreeRelationship(
      ...
      if (relationshipType === "parent" && existingParents.length === 1) {
        ...
        const freshRelationships = await storage.getFamilyTreeRelationships(
          treeId,
          userId,
        );

        // Check if marriage already exists between the two parents
        const existingMarriage = freshRelationships.find(
          (rel: any) =>
            rel.relationshipType === "marriage" &&
            ((rel.fromMemberId === firstParentId &&
              rel.toMemberId === savedMember.id) ||
              (rel.fromMemberId === savedMember.id &&
                rel.toMemberId === firstParentId)),
        );
        ...
        if (!existingMarriage) {
          ...
          const validatedMarriage =
            insertFamilyTreeRelationshipSchema.parse(marriageData);
      ...
      res
        .status(500)
        .json({ error: "Failed to create related family tree member" });
    }
  },
);
...
router.put(
  ...
  async (req: any, res) => {
    try {
      ...
      const validatedUpdates = insertFamilyTreeMemberSchema
        .partial()
        .parse(req.body);
      const updatedMember = await storage.updateFamilyTreeMember(
        memberId,
        userId,
        validatedUpdates,
      );
...
router.post(
  ...
  async (req: any, res) => {
    try {
      ...
      const validatedRelationship = insertFamilyTreeRelationshipSchema.parse({
        ...req.body,
        treeId,
      });
      ...
      if (
        validatedRelationship.fromMemberId === validatedRelationship.toMemberId
      ) {
        return res
          .status(400)
          .json({
            error: "A family member cannot have a relationship with themselves",
          });
      }
      ...
      const duplicateExists = existingRelationships.some(
        (rel: any) =>
          (rel.fromMemberId === validatedRelationship.fromMemberId &&
            rel.toMemberId === validatedRelationship.toMemberId) ||
          (rel.fromMemberId === validatedRelationship.toMemberId &&
            rel.toMemberId === validatedRelationship.fromMemberId),
      );
      ...
      if (duplicateExists) {
        return res
          .status(400)
          .json({
            error: "A relationship between these family members already exists",
          });
      }
      ...
      res
        .status(500)
        .json({ error: "Failed to create family tree relationship" });
    }
  },
);
...
router.get("/:treeId/relationships", readRateLimiter, async (req: any, res) => {
  try {
    ...
    const relationships = await storage.getFamilyTreeRelationships(
      treeId,
      userId,
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch family tree relationships" });
  }
});

router.put(
  "/:treeId/relationships/:relationshipId",
  writeRateLimiter,
  async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const validatedUpdates = insertFamilyTreeRelationshipSchema
        .partial()
        .parse(req.body);
      const updatedRelationship = await storage.updateFamilyTreeRelationship(
        relationshipId,
        userId,
        validatedUpdates,
      );
      ...
      res
        .status(500)
        .json({ error: "Failed to update family tree relationship" });
    }
  },
);
...
router.delete(
  ...
  async (req: any, res) => {
    try {
      ...
      await storage.deleteFamilyTreeRelationship(
        relationshipId,
        userId,
        treeId,
      );
      ...
      res
        .status(500)
        .json({ error: "Failed to delete family tree relationship" });
    }
  },
);
