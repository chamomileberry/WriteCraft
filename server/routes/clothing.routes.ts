...
router.get("/:id", readRateLimiter, async (req: any, res) => {
  try {
    ...
    const clothing = await storage.getClothing(req.params.id, userId, notebookId);
    if (!clothing) {
...
router.put("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const updatedClothing = await storage.updateClothing(req.params.id, userId, validatedUpdates);
