...
router.post(
  ...
  requireBooleanFeature("collaboration"),
  shareRateLimiter,
  async (req: any, res) => {
    try {
      ...
      if (userId !== validatedData.ownerId) {
        return res
          .status(403)
          .json({ message: "Only the owner can share resources" });
      }

      if (validatedData.userId === validatedData.ownerId) {
        return res
          .status(400)
          .json({ message: "Cannot share resource with yourself" });
      }
      ...
      if (!ownsResource) {
        console.warn(
          `[Security] Attempted to share unowned resource - userId: ${userId}, resourceType: ${validatedData.resourceType}, resourceId: ${validatedData.resourceId}`,
        );
  ...
  async (req: any, res) => {
    try {
      ...
      if (existingShare.ownerId !== userId) {
        return res
          .status(403)
          .json({ message: "Only the owner can remove shares" });
      }
    ...
    try {
      ...
      res.status(500).json({
        message:
          error instanceof Error
            ? error.message
            : "Failed to fetch shared resources",
      });
    }
  },
);
