...
router.post("/", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (notebookId) {
      ...
      if (!ownsNotebook) {
        console.warn(
          `[Security] Unauthorized map access attempt - userId: ${userId}, notebookId: ${notebookId}`,
        );
    ...
    if (error instanceof Error && error.message.includes("Unauthorized")) {
      ...
      console.warn(
        `[Security] Unauthorized map operation - userId: ${userId}, mapId: ${mapId}`,
      );
...
router.put("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const updatedMap = await storage.updateMap(
      req.params.id,
      userId,
      validatedUpdates,
    );
