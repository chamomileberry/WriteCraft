...
router.post(
  "/generate",
  aiRateLimiter,
  trackAIUsage("conflict_generation"),
  async (req: any, res) => {
    try {
      ...
      const { conflictType, genre, notebookId } = generateRequestSchema.parse(
        req.body,
      );
      ...
      const typeContext = conflictType
        ? ` focusing on ${conflictType} conflict`
        : "";
      const genreContext =
...
router.get("/:id", readRateLimiter, async (req: any, res) => {
  try {
    ...
    const conflict = await storage.getConflict(
      req.params.id,
      userId,
      notebookId,
    );
