...
router.post("/", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (notebookId) {
      ...
      if (!ownsNotebook) {
        console.warn(`[Security] Unauthorized music access attempt - userId: ${userId}, notebookId: ${notebookId}`);
    ...
    if (error instanceof Error && error.message.includes('Unauthorized')) {
      ...
      console.warn(`[Security] Unauthorized music operation - userId: ${userId}, musicId: ${musicId}`);
