...
router.put("/:id", validateInput(insertCanvasSchema.omit({ userId: true }).partial()), async (req: any, res) => {
  try {
    ...
    const updatedCanvas = await storage.updateCanvas(req.params.id, userId, updateData);
    ...
    if (error instanceof Error && error.message.includes('Unauthorized')) {
      ...
      console.warn(`[Security] Unauthorized canvas operation - userId: ${userId}, canvasId: ${canvasId}`);
