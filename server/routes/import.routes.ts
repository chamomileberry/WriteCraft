...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (isZip || isHtml || isRtf || isDocx || isPdf) {
      ...
      cb(
        new Error(
          "Only ZIP (World Anvil) or Campfire document files (HTML, RTF, DOCX, PDF) are allowed",
        ),
      );
    }
  },
...
function parseCampfireHTML(
  ...
  if (physicalPanel.length) {
    ...
    const physicalSection = physicalPanel.find(
      'h5:contains("Physical Features")',
    );
  ...
  const personalityPanel = $('h3:contains("Personality Traits")').next(
    ".list-panel",
  );
  ...
  const basicPanel = $('h3:contains("Basic Information")').next(
    ".custom-panel",
  );
  ...
  console.log(
    `[Campfire HTML] - Physical traits: ${Object.values(physicalTraits).flat().length}`,
  );
  console.log(
    `[Campfire HTML] - Personality traits: ${personalityTraits.length}`,
  );
  console.log(
    `[Campfire HTML] - Basic info fields: ${Object.keys(basicInfo).length}`,
  );
...
async function parseCampfireRTF(
  rtfBuffer: Buffer,
  filename: string,
): Promise<WorldAnvilArticle[]> {
...
async function parseCampfireDOCX(
  docxBuffer: Buffer,
  filename: string,
): Promise<WorldAnvilArticle[]> {
...
async function parseCampfirePDF(
  pdfBuffer: Buffer,
  filename: string,
): Promise<WorldAnvilArticle[]> {
  ...
  try {
    ...
    console.log(
      `[Campfire PDF] Converted ${lines.length} lines to HTML structure`,
    );
...
function parseWorldAnvilExport(zipBuffer: Buffer) {
  try {
    ...
    const articlesEntry = zipEntries.find((entry) =>
      entry.entryName.endsWith("articles.json"),
    );
    if (articlesEntry) {
      try {
        const articlesData = JSON.parse(
          articlesEntry.getData().toString("utf8"),
        );
        ...
        console.log(
          "[ZIP Parse] Could not parse articles.json:",
    ...
    if (articles.length === 0) {
      ...
      const articleFiles = zipEntries.filter(
        (entry) =>
          !entry.isDirectory &&
      ...
      console.log(
        `[ZIP Parse] Found ${articleFiles.length} individual article files`,
      );
      ...
      articleFiles.forEach((entry) => {
        try {
          ...
          if (data.title || data.id || data.name) {
            ...
            console.log(
              `[ZIP Parse] Skipping ${entry.entryName} - no title/id/name field`,
            );
          }
        } catch (e) {
          failed++;
          console.log(
            `[ZIP Parse] Could not parse ${entry.entryName}:`,
            e instanceof Error ? e.message : "Unknown error",
          );
        }
      });
      ...
      console.log(
        `[ZIP Parse] Successfully parsed ${parsed} articles, failed to parse ${failed} files`,
      );
    }
...
function mapArticleToContent(
  article: WorldAnvilArticle,
  userId: string,
  notebookId: string,
  importSource: string = "world_anvil",
) {
  ...
  if (article.entityClass) {
    ...
    article.template &&
    typeof article.template === "object" &&
  ...
  console.log(
    `[Type Mapping] Article "${article.title}": typeKey="${typeKey}" → contentType="${contentType}"`,
  );

  // Log unmapped types to help debug
  if (!WORLD_ANVIL_TYPE_MAPPING[typeKey] && typeKey !== "document") {
    console.log(
      `[Type Mapping] Unmapped type "${typeKey}" for article "${article.title}" - defaulting to document`,
    );
  }
  ...
  const extractNamesFromTitle = (
    title: string,
  ): { givenName: string; familyName: string; honorificTitle: string } => {
    ...
    for (const candidate of multiWordTitles) {
      ...
      if (parts.length >= candidateWords.length) {
        const titlePrefix = parts
          .slice(0, candidateWords.length)
          .join(" ")
          .toLowerCase();
  ...
  const parseArray = (value: string | string[] | undefined): string[] => {
    ...
    if (Array.isArray(value)) {
      return value
        .map((item) =>
          typeof item === "string" ? stripBBCode(item) : String(item),
        )
        .filter(Boolean);
    }
  ...
  if (contentType === "character") {
    ...
    const firstNameValue = extractField(
      article,
      "firstname",
      "firstName",
      "first_name",
    ...
    const lastNameValue = extractField(
      article,
      "lastname",
      "lastName",
      "last_name",
    ...
    const honorificValue = extractField(
      article,
      "honorific",
      "title",
      "prefix",
    ...
    const extractedNames =
      !firstNameValue && !lastNameValue
        ? extractNamesFromTitle(article.title)
        : { givenName: "", familyName: "", honorificTitle: "" };
    ...
    const characterData = {
      ...
      middleName:
        extractField(article, "middlename", "middleName", "middle_name") || "",
      maidenName:
        extractField(article, "maidenname", "maidenName", "maiden_name") || "",
      ...
      // Physical attributes
      physicalDescription: extractField(
        article,
        "physique",
        "physicalDescription",
        "physicaldescription",
      ...
      distinctiveBodyFeatures: extractField(
        article,
        "bodyFeatures",
        "bodyfeatures",
        "body_features",
      ...
      supernaturalPowers: extractField(
        article,
        "specialAbilities",
        "specialabilities",
        "special_abilities",
      ...
      physicalCondition: extractField(
        article,
        "sidepanelcontenttop",
        "physicalCondition",
        "physicalcondition",
      ...
      intellectualTraits: extractField(
        article,
      ...
      valuesEthicsMorals: extractField(
        article,
        "morality",
        "values",
        "ethics",
      ...
      sexualOrientation: extractField(
        article,
        "sexuality",
        "sexualOrientation",
        "sexualorientation",
    ...
    return {
      ...
      intelligence: extractField(
        article,
        "intelligence",
        "intellect",
        "cognitive",
      ),
      socialStructure: extractField(
        article,
        "socialStructure",
        "socialstructure",
        "society",
    ...
    return {
      ...
      name: article.title || "Untitled",
      locationType:
        extractField(article, "locationType", "locationtype", "type") ||
        "other",
      description: extractField(
        article,
        "description",
    ...
    return {
      ...
      name: article.title || "Untitled",
      organizationType:
        extractField(article, "organizationType", "organizationtype", "type") ||
        "other",
      purpose:
        extractField(article, "purpose", "mission", "goals") ||
      ...
      structure: extractField(
        article,
        "structure",
        "hierarchy",
        "organization",
      ),
      leadership: extractField(
        article,
        "leadership",
        "leaders",
        "commander",
    ...
    return {
      ...
      workEnvironment: extractField(
        article,
        "workEnvironment",
        "workenvironment",
        "environment",
      ),
      trainingRequired: extractField(
        article,
        "trainingRequired",
        "trainingrequired",
        "training",
      ...
      physicalDemands: extractField(
        article,
        "physicalDemands",
        "physicaldemands",
        "physical",
      ),
      mentalDemands: extractField(
        article,
        "mentalDemands",
        "mentaldemands",
        "mental",
    ...
    return {
      ...
      name: article.title || "Untitled",
      settlementType:
        extractField(article, "settlementType", "settlementtype", "type") ||
        "town",
      description: extractField(
        article,
        "description",
    ...
    return {
      ...
      name: article.title || "Untitled",
      ritualType:
        extractField(article, "ritualType", "ritualtype", "type", "category") ||
        "other",
      description: extractField(
        article,
        "description",
    ...
    return {
      ...
      name: article.title || "Untitled",
      lawType:
        extractField(article, "lawType", "lawtype", "type", "category") ||
        "regulation",
      description: extractField(
        article,
        "description",
    ...
    return {
      ...
      name: article.title || "Untitled",
      itemType:
        extractField(article, "itemType", "itemtype", "type", "category") ||
        "other",
      description: extractField(
        article,
        "description",
    ...
    return {
      ...
      accessibility: extractField(
        article,
        "accessibility",
        "access",
        "availability",
      ),
      secrets: extractField(
        article,
        "secrets",
        "hiddenInfo",
        "hiddeninfo",
    ...
    return {
      ...
      name: article.title || "Untitled",
      rankType:
        extractField(article, "rankType", "ranktype", "type", "category") ||
        "other",
      description: extractField(
        article,
        "description",
    ...
    return {
      ...
      historicalOutbreaks: extractField(
        article,
        "historicalOutbreaks",
        "historicaloutbreaks",
        "history",
  ...
  async (req: any, res) => {
    try {
      ...
      if (notebookId) {
        ...
        if (!notebook) {
          console.log(
            `[Import] Invalid notebookId ${notebookId} for user ${userId}, will create new notebook`,
          );
          notebookId = null;
        }
      }
      ...
      console.log(
        `[Import] Final notebookId: ${notebookId} for user: ${userId}`,
      );
      ...
      for (const file of files) {
        ...
        const isZip =
          ...
          filename.endsWith(".zip");
        const isHtml =
          file.mimetype === "text/html" || filename.endsWith(".html");
        const isRtf =
          file.mimetype === "application/rtf" ||
          file.mimetype === "text/rtf" ||
          filename.endsWith(".rtf");
        ...
        if (isZip) {
          console.log(
            `[Import] Processing World Anvil ZIP: ${file.originalname}`,
          );
          ...
          console.log(
            `[Import] Processing Campfire HTML: ${file.originalname}`,
          );
          ...
          const articles = await parseCampfireRTF(
            file.buffer,
            file.originalname,
          );
          allArticles.push(...articles);
          ...
          console.log(
            `[Import] Processing Campfire DOCX: ${file.originalname}`,
          );
          const articles = await parseCampfireDOCX(
            file.buffer,
            file.originalname,
          );
          allArticles.push(...articles);
          ...
          const articles = await parseCampfirePDF(
            file.buffer,
            file.originalname,
          );
          allArticles.push(...articles);
      ...
      console.log(
        `[Import] Total articles from all files: ${allArticles.length}`,
      );
...
async function processImport(
  ...
  try {
    ...
    console.log(
      `[Import ${jobId}] Starting import of ${parsed.totalItems} articles`,
    );

    // Batch fetch ALL existing saved items for this notebook in ONE query (Issue 1 fix)
    const existingItems = await storage.getSavedItemsByNotebookBatch(
      userId,
      notebookId,
    );
    console.log(
      `[Import ${jobId}] Found ${existingItems.length} existing saved items in notebook`,
    );
    ...
    const existingTitlesSet = new Set(
      existingItems
        .map((item) => {
          ...
          return displayName
            ? `${itemType}:${displayName.toLowerCase().trim()}`
            : null;
        })
    ...
    console.log(
      `[Import ${jobId}] Built duplicate detection Sets: ${existingIdsSet.size} IDs, ${existingTitlesSet.size} titles`,
    );
    ...
    for (let i = 0; i < parsed.articles.length; i++) {
      ...
      try {
        ...
        if (i < 3) {
          console.log(
            `[Import ${jobId}] Article ${i + 1} structure:`,
            JSON.stringify(
              {
                title: article.title,
        ...
        const mapped = mapArticleToContent(
          article,
          userId,
          notebookId,
          importSource,
        ...
        if (article.entityClass) {
          contentType =
            WORLD_ANVIL_TYPE_MAPPING[article.entityClass.toLowerCase()] ||
            "document";
        } else if (article.templateType) {
          contentType =
            WORLD_ANVIL_TYPE_MAPPING[article.templateType.toLowerCase()] ||
            "document";
        } else if (
          article.template &&
          typeof article.template === "object" &&
          ...
          contentType =
            WORLD_ANVIL_TYPE_MAPPING[article.template.title.toLowerCase()] ||
            "document";
        } else if (
          article.category &&
          typeof article.category === "object" &&
          ...
          contentType =
            WORLD_ANVIL_TYPE_MAPPING[article.category.title.toLowerCase()] ||
            "document";
        } else if (article.type) {
          contentType =
            WORLD_ANVIL_TYPE_MAPPING[article.type.toLowerCase()] || "document";
        }

        console.log(
          `[Import ${jobId}] Processing ${i + 1}/${parsed.totalItems}: "${article.title}" → ${contentType}`,
        );
        ...
        if (contentType === "character") {
          console.log(
            `[Import ${jobId}] Processing character: ${article.title}`,
          );
          ...
          console.log(`[Import ${jobId}] Mapped fields:`, {
            ...
            description: charData.description
              ? `${charData.description.substring(0, 50)}...`
              : "none",
          ...
          console.log(
            `[Import ${jobId}] ✓ Created character: ${article.title}`,
          );
        } else if (contentType === "location") {
          ...
          results.imported.push(createdItem.id);
          console.log(
            `[Import ${jobId}] ✓ Created organization: ${article.title}`,
          );
        } else if (contentType === "species") {
          // Check if species with this name already exists in the notebook
          const existingSpecies = await storage.findSpeciesByName(
            article.title,
            notebookId,
          );
          ...
          if (existingSpecies) {
            ...
            console.log(
              `[Import ${jobId}] ↻ Reusing existing species: ${article.title}`,
            );
          } else {
            // Create new species
            ...
            results.imported.push(createdItem.id);
            console.log(
              `[Import ${jobId}] ✓ Created species: ${article.title}`,
            );
          }
        } else if (contentType === "profession") {
          ...
          results.imported.push(createdItem.id);
          console.log(
            `[Import ${jobId}] ✓ Created profession: ${article.title}`,
          );
        } else if (contentType === "ethnicity") {
          createdItem = await storage.createEthnicity(mapped as any);
          results.imported.push(createdItem.id);
          console.log(
            `[Import ${jobId}] ✓ Created ethnicity: ${article.title}`,
          );
        } else if (contentType === "settlement") {
          createdItem = await storage.createSettlement(mapped as any);
          results.imported.push(createdItem.id);
          console.log(
            `[Import ${jobId}] ✓ Created settlement: ${article.title}`,
          );
        } else if (contentType === "ritual") {
          ...
          results.imported.push(createdItem.id);
          console.log(
            `[Import ${jobId}] ✓ Created transportation: ${article.title}`,
          );
        } else if (contentType === "rank") {
          ...
          results.imported.push(createdItem.id);
          console.log(
            `[Import ${jobId}] ✓ Created condition: ${article.title}`,
          );
        } else {
          ...
          console.log(
            `[Import ${jobId}] ⊘ Skipped (unsupported type): ${skipReason}`,
          );
        }
        ...
        if (createdItem) {
          try {
            ...
            const titleKey = displayName
              ? `${contentType}:${displayName.toLowerCase().trim()}`
              : null;

            // Check both ID and title for duplicates
            const isDuplicateById = existingIdsSet.has(itemKey);
            const isDuplicateByTitle = titleKey
              ? existingTitlesSet.has(titleKey)
              : false;
            ...
            if (isDuplicateById) {
              ...
              results.skipped.push(skipReason);
              console.log(
                `[Import ${jobId}] ⊘ Skipped duplicate (ID match): ${skipReason}`,
              );
              ...
              results.skipped.push(skipReason);
              console.log(
                `[Import ${jobId}] ⊘ Skipped duplicate (title match): ${skipReason}`,
              );
            } else {
              ...
              console.log(
                `[Import ${jobId}] ✓ Created saved_item ${savedItem.id} for ${contentType} "${article.title}" in notebook ${notebookId}`,
              );
            }
          } catch (saveError) {
            console.error(
              `[Import ${jobId}] ✗ Failed to check/create saved_item for ${contentType} "${article.title}":`,
              saveError,
            );
        ...
        if ((i + 1) % 10 === 0 || i === parsed.totalItems - 1) {
          ...
          console.log(
            `[Import ${jobId}] Progress update: ${i + 1}/${parsed.totalItems} (${progress}%)`,
          );
        ...
        console.error(
          `[Import ${jobId}] ✗ Failed: ${article.title} - ${errorMsg}`,
        );
      }
    }

    console.log(
      `[Import ${jobId}] Completed: ${results.imported.length} imported, ${results.skipped.length} skipped, ${results.failed.length} failed`,
    );
    ...
    console.log(
      `[Import ${jobId}] Import job marked as completed in database with status: completed`,
    );
  } catch (error) {
    await storage.updateImportJob(jobId, {
      status: "failed",
