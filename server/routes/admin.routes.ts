...
router.get("/characters/issues", readRateLimiter, async (req, res) => {
  try {
    ...
    const hasAccess = await storage.validateNotebookOwnership(
      notebookId,
      userId,
    );
    ...
    if (!hasAccess) {
      return res
        .status(403)
        .json({ error: "You do not have access to this notebook" });
    }
    ...
    res.json({
      ...
      stats: {
        ...
        totalIssues:
          issues.missingFamilyName.length +
          issues.missingDescription.length +
          issues.missingImage.length,
      },
    });
  } catch (error) {
...
router.get("/characters/duplicates", readRateLimiter, async (req, res) => {
  try {
    ...
    const duplicateGroups = await storage.getPotentialDuplicates(
      userId,
      notebookId,
    );
    ...
    res.json({
      ...
      stats: {
        ...
        totalCharacters: duplicateGroups.reduce(
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const result = await storage.bulkDeleteCharactersWithIssues(
        userId,
        notebookId,
      );
