  ...
  async (req, res, next) => {
    try {
      ...
      if (!teamSubscription) {
        return res
          .status(403)
          .json({ message: "Only team members can invite others" });
      }
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      // Get user's team subscription
      const teamSubscription =
        await teamService.getUserTeamSubscription(currentUserId);
      ...
      if (!isOwner) {
        return res
          .status(403)
          .json({ message: "Only team owner can remove members" });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!isOwner) {
        return res
          .status(403)
          .json({ message: "Only team owner can update member roles" });
      }
