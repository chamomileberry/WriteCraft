...
router.post("/", writeRateLimiter, requireFeature('create_notebook'), validateInput(insertNotebookSchema.omit({ userId: true })), async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
...
// Update a notebook
router.put("/:id", writeRateLimiter, validateInput(updateNotebookSchema), async (req: any, res) => {
  try {
    const userId = req.user.claims.sub;
