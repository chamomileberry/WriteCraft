...
import {
  requireTeamTier,
  requirePermission,
} from "../middleware/teamPermissions";
...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      const totalAIOperations = aiUsageStats.reduce(
        (sum, stat) => sum + Number(stat.count),
        0,
      );
      const totalAICost = aiUsageStats.reduce(
        (sum, stat) => sum + Number(stat.totalCost),
        0,
      );
      const totalTokens = aiUsageStats.reduce(
        (sum, stat) =>
          sum + Number(stat.totalInputTokens) + Number(stat.totalOutputTokens),
        0,
      );

      // Group AI usage by operation type
      const usageByOperation = aiUsageStats.reduce(
        (acc, stat) => {
      ...
      const memberActivityMap = members.map((member) => {
        const memberAIUsage = aiUsageStats.filter(
          (s) => s.userId === member.userId,
        );
        const memberActivity = activityStats.filter(
          (s) => s.userId === member.userId,
        );
        ...
        return {
          ...
          role: member.role,
          aiOperations: memberAIUsage.reduce(
            (sum, s) => sum + Number(s.count),
            0,
          ),
          activities: memberActivity.reduce(
            (sum, s) => sum + Number(s.count),
            0,
          ),
      ...
      res.json({
        ...
        usageByOperation: Object.entries(usageByOperation).map(
          ([type, data]) => ({
            operationType: type,
            count: data.count,
