...
router.post(
  ...
  async (req: any, res) => {
    try {
      ...
      const { genre, gender, ethnicity, notebookId } =
      ...
      const aiResult = await generateCharacterWithAI({
        genre,
        gender,
        ethnicity,
      });
      ...
      if (error instanceof Error && error.message.includes("Unauthorized")) {
        ...
        console.warn(
          `[Security] Unauthorized error in character generation - userId: ${userId}`,
        );
...
router.post(
  "/:id/generate-field",
  aiRateLimiter,
  trackAIUsage("character_field_generation"),
  async (req: any, res) => {
    try {
      ...
      const userId = req.user.claims.sub;
      const { fieldName, currentFormData, notebookId } =
        generateFieldSchema.parse(req.body);
      ...
      const aiResult = await generateCharacterFieldWithAI(
        fieldName,
        contextCharacter,
      );
...
router.post(
  "/",
  writeRateLimiter,
  validateInput(insertCharacterSchema.omit({ userId: true })),
  async (req: any, res) => {
    try {
...
router.patch("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    try {
      await storage.updateSavedItemDataByItem(
        userId,
        "character",
        req.params.id,
        notebookId,
      ...
      console.warn(
        "Failed to update saved item data after character update:",
        error,
      );
