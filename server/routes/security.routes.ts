...
router.get('/overview', requireAdmin, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      highAlerts: unacknowledgedAlerts.filter(a => a.severity === 'HIGH').length,
...
router.get('/alerts', requireAdmin, async (req, res) => {
  try {
    ...
    const alerts = await IntrusionDetectionService.getUnacknowledgedAlerts(limit);
    
    // Filter by acknowledged status if specified
    const filteredAlerts = acknowledgedFilter !== undefined 
...
router.post('/block-ip', requireAdmin, async (req: any, res) => {
  try {
    ...
    res.json({ success: true, message: `IP ${data.ipAddress} blocked successfully` });
  } catch (error) {
...
router.post('/unblock-ip', requireAdmin, async (req: any, res) => {
  try {
    ...
    res.json({ success: true, message: `IP ${data.ipAddress} unblocked successfully` });
  } catch (error) {
