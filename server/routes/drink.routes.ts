...
router.get("/user/:userId?", async (req: any, res) => {
  try {
    ...
    const drinks = await storage.getUserDrinks(userId, notebookId);
    res.json(drinks);
  } catch (error) {
...
router.get("/:id", async (req: any, res) => {
  try {
    ...
    if (drink.userId !== userId) {
      return res.status(403).json({ error: 'Forbidden: You do not own this drink' });
    }
    res.json(drink);
  } catch (error) {
...
router.put("/:id", async (req: any, res) => {
  try {
    ...
    const updatedDrink = await storage.updateDrink(req.params.id, userId, validatedUpdates);
