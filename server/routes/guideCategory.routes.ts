...
router.post("/", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (!userIsAdmin) {
      console.warn(
        `[Security] Non-admin user attempted to create category - userId: ${userId}`,
      );
      return res
        .status(403)
        .json({ error: "Only administrators can create categories" });
    }
...
router.put("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (!userIsAdmin) {
      console.warn(
        `[Security] Non-admin user attempted to update category - userId: ${userId}`,
      );
      return res
        .status(403)
        .json({ error: "Only administrators can update categories" });
    }

    const validatedCategory = insertGuideCategorySchema
      .partial()
      .parse(req.body);

    // Check for cycles if parentId is being updated
    if (
      validatedCategory.parentId !== undefined &&
      validatedCategory.parentId !== null
    ) {
      ...
      if (
        isDescendant(req.params.id, validatedCategory.parentId, allCategories)
      ) {
        return res
          .status(400)
          .json({
    ...
    const updatedCategory = await storage.updateGuideCategory(
      req.params.id,
      validatedCategory,
    );
...
router.delete("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (!userIsAdmin) {
      console.warn(
        `[Security] Non-admin user attempted to delete category - userId: ${userId}`,
      );
      return res
        .status(403)
        .json({ error: "Only administrators can delete categories" });
    }
...
router.post("/reorder", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    if (!userIsAdmin) {
      console.warn(
        `[Security] Non-admin user attempted to reorder categories - userId: ${userId}`,
      );
      return res
        .status(403)
        .json({ error: "Only administrators can reorder categories" });
    }
