...
router.get("/user-data", exportRateLimiter, async (req: any, res) => {
  try {
    ...
    const [
      ...
      Promise.all(
        notebooks.map((nb) => storage.getUserPinnedContent(userId, nb.id)),
    ...
    const [subscription, alerts, discountUsage] = await Promise.all([
      db
        ...
        .limit(1),
      db.select().from(billingAlerts).where(eq(billingAlerts.userId, userId)),
      db
        .select()
    ...
    const exportData = {
      exportMetadata: {
        ...
        version: "2.0",
        description:
          "Complete GDPR data export including all user-generated content, preferences, and usage data",
      },
