  ...
  try {
    ...
    if (!hasAccess) {
      return res.status(403).json({
        ...
        message:
          "API access is only available for Professional and Team tier subscribers",
        upgradeUrl: "/pricing",
      });
    }
    ...
    const monthlyRateLimit =
      validatedData.monthlyRateLimit ||
    ...
    res.json({
      ...
      key: result.key,
      message:
        "API key created successfully. Save this key securely - you won't be able to see it again!",
    });
  } catch (error) {
...
router.post("/:id/rotate", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    res.json({
      ...
      key: result.key,
      message:
        "API key rotated successfully. The old key has been revoked. Save this new key securely!",
    });
  } catch (error) {
