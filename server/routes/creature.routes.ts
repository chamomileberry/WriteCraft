...
router.post(
  "/generate",
  aiRateLimiter,
  trackAIUsage("creature_generation"),
  async (req: any, res) => {
    try {
      ...
      const { genre, creatureType, notebookId } = generateRequestSchema.parse(
        req.body,
      );
...
router.patch(
  "/:id",
  writeRateLimiter,
  validateInput(insertCreatureSchema.omit({ userId: true }).partial()),
  async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;

      const updatedCreature = await storage.updateCreature(
        req.params.id,
        userId,
        req.body,
      );
