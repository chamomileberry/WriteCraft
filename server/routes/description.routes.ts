...
router.post(
  "/generate",
  aiRateLimiter,
  trackAIUsage("description_generation"),
  async (req: any, res) => {
    try {
...
router.post("/", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const savedDescription =
      await storage.createDescription(validatedDescription);
...
router.get("/:id", readRateLimiter, async (req: any, res) => {
  try {
    ...
    const description = await storage.getDescription(
      req.params.id,
      userId,
      notebookId,
    );
...
router.put("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const updatedDescription = await storage.updateDescription(
      req.params.id,
      userId,
      validatedUpdates,
    );
