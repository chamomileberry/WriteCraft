...
router.get("/:id", async (req, res) => {
  try {
    const music = await storage.getMusic(req.params.id);
    if (!music) {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updatedMusic = await storage.updateMusic(req.params.id, validatedUpdates);
...
router.delete("/:id", async (req, res) => {
  try {
    await storage.deleteMusic(req.params.id);
