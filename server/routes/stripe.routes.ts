  ...
  async (req: any, res) => {
    try {
      ...
      if (userEmail && subscription.currentPeriodEnd) {
        ...
        const tierName =
          (subscription.tier || "Unknown").charAt(0).toUpperCase() +
          (subscription.tier || "unknown").slice(1);
...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch invoices", message: error.message });
    }
  },
);
...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch invoice", message: error.message });
    }
  },
);
  ...
  async (req: any, res) => {
    try {
      ...
      if (subscription.pausedAt) {
        return res
          .status(400)
          .json({ error: "Subscription is already paused" });
      }
      ...
      if (resumeAt) {
        ...
        if (resumeDate <= new Date()) {
          return res
            .status(400)
            .json({ error: "Resume date must be in the future" });
        }
      }
...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      // Get pause status
      const pauseStatus = await stripeService.getPauseStatus(
        subscription.stripeSubscriptionId,
      );
      ...
      res
        .status(500)
        .json({ error: "Failed to get pause status", message: error.message });
    }
  },
);
...
router.post(
  ...
  async (req: any, res) => {
    try {
      ...
      let customerId = hasStripeData(subscription)
        ? subscription.stripeCustomerId
        : undefined;
...
router.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Webhook handler failed", message: error.message });
  }
});
