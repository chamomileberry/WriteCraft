...
router.delete("/account", async (req: any, res) => {
  try {
    ...
    logger.info(`Account successfully deleted for user ${userId} (${userEmail})`);
...
router.post("/account/deletion-request", async (req: any, res) => {
  try {
    ...
    logger.info(`Account deletion request submitted for user ${userId} (${userEmail})`);
    ...
    res.json({ 
      success: true, 
      message: "Deletion request received. Your account will be deleted immediately when you confirm.",
      nextSteps: "To permanently delete your account, use the DELETE /api/users/account endpoint"
    });
  } catch (error) {
