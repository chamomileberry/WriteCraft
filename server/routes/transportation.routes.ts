...
router.post("/", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const savedTransportation = await storage.createTransportation(
      validatedTransportation,
    );
...
router.get("/user/:userId?", readRateLimiter, async (req: any, res) => {
  try {
    ...
    const transportations = await storage.getUserTransportation(
      userId,
      notebookId,
    );
...
router.put("/:id", writeRateLimiter, async (req: any, res) => {
  try {
    ...
    const updatedTransportation = await storage.updateTransportation(
      req.params.id,
      userId,
      validatedUpdates,
    );
