...
export function requireFeature(action: string) {
  return async (req: any, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      console.error(
        `[Feature Gate] Error checking permission for action '${action}':`,
        error,
      );
      return res.status(500).json({
        error: "Failed to verify feature access",
        message:
          "An error occurred while checking your subscription limits. Please try again.",
      });
    }
  };
}
 ...
 * @returns Express middleware function
 */
export function requireBooleanFeature(
  ...
  return async (req: any, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      console.error(
        `[Feature Gate] Error checking feature '${feature}':`,
        error,
      );
      return res.status(500).json({
        error: "Failed to verify feature access",
