...
router.patch(
  ...
  async (req: any, res) => {
    try {
      ...
      console.log(
        `[SECURITY] User ${authenticatedUserId} successfully updated their profile`,
      );
...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      const responseData = requestingUser.isAdmin
        ? user
        : {
            id: user.id,
            firstName: user.firstName,
...
router.get(
  ...
  (req: any, res) => {
    ...
    if (!token) {
      console.error(
        "[CSRF] Failed to generate CSRF token - Lusca may not be properly configured",
      );
 ...
 */
router.post("/auth/logout", secureAuthentication, (req: any, res) => {
  const userId = req.user?.claims?.sub;
