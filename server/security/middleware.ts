 ...
 */
export const secureAuthentication: RequestHandler = async (
  req: any,
  res,
  next,
) => {
  try {
    ...
    if (
      ...
      console.error(
        `[SECURITY] Test mode bypass attempt in production - IP: ${req.ip}`,
      );
...
export function createRateLimiter(options?: {
  ...
  const maxRequests =
    options?.maxRequests || SECURITY_CONFIG.RATE_LIMIT_MAX_REQUESTS;
  ...
  static middleware(): RequestHandler {
    return (req: any, res: Response, next: NextFunction) => {
      ...
      if (!csrfToken || !this.validateToken(sessionId, csrfToken)) {
        console.warn(
          `[SECURITY] CSRF validation failed for session ${sessionId}`,
        );
 ...
 */
export const generateCSPNonce: RequestHandler = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export function sanitizeInput(input: any, fieldName?: string): any {
  if (typeof input === "string") {
    ...
    const sqlInjectionPattern =
      /(';|--;|\*\/|\/\*|xp_|sp_|exec\s*\(|execute\s*\(|union\s+select|insert\s+into|delete\s+from|drop\s+table|update\s+\w+\s+set)/gi;

    if (sqlInjectionPattern.test(input)) {
      console.warn(
        `[SECURITY] Potential SQL injection attempt detected: ${input.substring(0, 100)}`,
      );
 ...
 */
export const sanitizeAllInputs: RequestHandler = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
    ...
    switch (resourceType) {
      case "notebook": {
        const [notebook] = await db
          ...
          .from(notebooks)
          .where(
            and(eq(notebooks.id, resourceId), eq(notebooks.userId, userId)),
          )
          .limit(1);
        return !!notebook;
      }
...
export function enforceRowLevelSecurity(
  ...
  const notebookId = req.query['notebookId'] || req.body?.notebookId;
    ...
    // Log access attempt
    console.log(
      `[RLS] User ${userId} attempting to access ${resourceType}:${resourceId || "unknown"}`,
    );
    ...
    if (resourceId) {
      ...
      if (isOwner) {
        hasAccess = true;
        console.log(
          `[RLS] User ${userId} is owner of ${resourceType}:${resourceId}`,
        );
      } else {
        ...
        if (share) {
          ...
          hasAccess = true;
          console.log(
            `[RLS] User ${userId} has ${permission} access to shared ${resourceType}:${resourceId}`,
          );
        }
      }
...
export async function canAccessResource(
  ...
  if (share) {
    ...
    if (requiredPermission) {
      ...
      const userLevel =
        permissionLevels[share.permission as keyof typeof permissionLevels] ||
        0;
