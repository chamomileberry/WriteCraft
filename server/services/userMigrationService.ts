...
class UserMigrationService {
    ...
    const usageSummaries = await db
      ...
      .where(
        and(
          ...
          gte(aiUsageDailySummary.date, thirtyDaysAgo.toISOString().split('T')[0])
        )
      )
      .orderBy(desc(aiUsageDailySummary.date));
    
    const daysActive = usageSummaries.length;
    const totalAIGenerations = usageSummaries.reduce((sum, day) => sum + (day.totalOperations || 0), 0);
    const avgDailyAIGenerations = daysActive > 0 ? Math.round(totalAIGenerations / daysActive) : 0;
    const maxDailyAIGenerations = usageSummaries.length > 0 
  ...
  private recommendTier(usage: {
    ...
    totalAIGenerations: number;
    daysActive: number;
  }): { recommendedTier: 'free' | 'author' | 'professional'; confidence: 'high' | 'medium' | 'low'; reasoning: string[] } {
    ...
    // Free tier
    const hasAnyUsage = usage.totalProjects > 0 || usage.totalNotebooks > 0 || usage.totalAIGenerations > 0;
    
    if (!hasAnyUsage) {
      reasoning.push('No significant usage detected');
      reasoning.push('New or inactive user');
    } else {
      reasoning.push(`Light usage: ${usage.totalProjects} projects, ${usage.totalNotebooks} notebooks`);
      reasoning.push(`Low AI usage: ${usage.avgDailyAIGenerations} avg generations/day`);
    }
   ...
   */
  async migrateUser(userId: string, analysis?: UserUsageAnalysis): Promise<void> {
    ...
    if (existingSub && existingSub.tier !== 'free') {
      ...
      console.log(`[Migration] User ${userId} already has ${existingSub.tier} tier - skipping`);
      return;
    }
    ...
    console.log(`[Migration] Assigned user ${userId} to ${analysis.recommendedTier} tier (${analysis.confidence} confidence)`);
  }
  ...
  async migrateAllUsers(): Promise<MigrationStats> {
    ...
    for (const userId of Array.from(allUserIds)) {
      try {
        ...
        if (stats.usersAnalyzed % 10 === 0) {
          console.log(`[Migration] Progress: ${stats.usersAnalyzed}/${stats.totalUsers} users migrated`);
        }
      } catch (error) {
    ...
    console.log(`[Migration] Complete! Migrated ${stats.usersAnalyzed}/${stats.totalUsers} users`);
  ...
  async previewMigration(): Promise<{
    ...
    for (const userId of Array.from(allUserIds)) {
      try {
        ...
        console.error(`[Migration Preview] Failed to analyze user ${userId}:`, error);
      }
    }
    ...
    return {
      ...
      recommendations: recommendations.sort((a, b) => 
        b.totalAIGenerations - a.totalAIGenerations
      ),
      tierDistribution
    };
  }
