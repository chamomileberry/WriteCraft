...
export class SubscriptionService {
  ...
  async getUserSubscription(
    userId: string,
  ): Promise<SubscriptionWithComputedFields> {
    if (!userId) {
    ...
    if (!subscription) {
      console.log(
        "[Subscription] No subscription found for user, creating free tier:",
        userId,
      );
    ...
    const effectiveTier =
      subscription.pausedAt && subscription.tier !== "free"
        ? "free"
    ...
    switch (action) {
      case "create_project":
        ...
        if (!projectLimitExceeded) {
          ...
          if (
            (graceStatus.inGracePeriod || graceStatus.expired) &&
            (await this.isUnderAllLimits(userId))
          ) {
   ...
   */
  async getMonthlyPremiumUsage(
    userId: string,
    operationType: "polish" | "extended_thinking",
  ): Promise<number> {
  ...
  async canUsePremiumOperation(
    ...
    // Check tier access
    const isProfessionalOrTeam =
      subscription.effectiveTier === "professional" ||
      subscription.effectiveTier === "team";
  ...
  async getUsageForecast(userId: string) {
    ...
    const totalOps = last7Days.reduce(
    ...
    if (dailyLimit !== null && dailyLimit > 0 && averageDailyUsage > 0) {
      ...
      const usagePercentage = Math.round(
        (averageDailyUsage / dailyLimit) * 100,
      );
      ...
      // High usage - estimate when limit will be hit
        ...
        const recentDays = last7Days
          .slice(-3)
          .map((d) => d.totalOperations ?? 0);
        if (recentDays.length >= 2) {
          ...
          if (usageGrowthRate > 0 && avgRecentUsage < dailyLimit) {
            const daysToLimit = Math.floor(
              (dailyLimit - avgRecentUsage) / usageGrowthRate,
            );
  ...
  async getTodayUsage(userId: string) {
    ...
    const remaining =
      dailyLimit === -1 ? -1 : Math.max(0, dailyLimit - currentUsage);
    ...
    return {
      ...
      tier: subscription.tier,
      usagePercentage:
        dailyLimit === -1 ? 0 : Math.round((currentUsage / dailyLimit) * 100),
      details: {
  ...
  async startGracePeriod(userId: string, limitType?: string): Promise<void> {
    ...
    console.log(
      `[Grace Period] Started for user ${userId}, expires ${gracePeriodEnd.toISOString()}`,
    );
  ...
  async getSubscriptionStatus(userId: string) {
    ...
    const projectLimitExceeded =
      ...
      projectCount >= subscription.limits.maxProjects;
    const notebookLimitExceeded =
      subscription.limits.maxNotebooks !== null &&
      notebookCount >= subscription.limits.maxNotebooks;
    const aiLimitExceeded =
      subscription.limits.aiGenerationsPerDay !== null &&
    ...
    if (graceStatus.inGracePeriod) {
      warnings.push(
        `Grace period active: ${graceStatus.daysRemaining} days remaining to upgrade or reduce usage`,
      );

      if (projectLimitExceeded) {
        warnings.push(
          `Project limit exceeded: ${projectCount}/${subscription.limits.maxProjects} projects`,
        );
      }
      if (notebookLimitExceeded) {
        warnings.push(
          `Notebook limit exceeded: ${notebookCount}/${subscription.limits.maxNotebooks} notebooks`,
        );
      }
      if (aiLimitExceeded) {
        warnings.push(
          `AI generation limit exceeded: ${todayAIUsage}/${subscription.limits.aiGenerationsPerDay} generations today`,
        );
      }
    }
    ...
    if (graceStatus.expired) {
      warnings.push(
        "Grace period expired: Please upgrade or reduce usage to continue",
      );
    }
