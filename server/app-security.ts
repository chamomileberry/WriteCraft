...
export function applySecurityMiddleware(app: Express): void {
  ...
  console.log(
    `[SECURITY] ${enableIDS && !isDevelopment ? "✓" : "○"} Intrusion Detection System (IDS) ${enableIDS && !isDevelopment ? "active" : "disabled (set ENABLE_IDS=true to enable)"}`,
  );
  console.log(
    `[SECURITY] ${enableIDS && !isDevelopment ? "✓" : "○"} IP blocking ${enableIDS && !isDevelopment ? "enabled" : "disabled (set ENABLE_IDS=true to enable)"}`,
  );
  console.log(
    `[SECURITY] ✓ Rate limiting enabled (${isDevelopment ? "10000" : "1000"} req/15min)`,
  );
  console.log(
    `[SECURITY] ${enableIDS && !isDevelopment ? "✓" : "○"} Injection detection ${enableIDS && !isDevelopment ? "active" : "disabled (set ENABLE_IDS=true to enable)"}`,
  );
