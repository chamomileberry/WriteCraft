...
export class ShareRepository extends BaseRepository {
  ...
  async updateShare(id: string, userId: string, updates: Partial<InsertShare>): Promise<Share> {
    ...
    if (existing.ownerId !== userId) {
      throw new Error('Unauthorized: Only the owner can modify share permissions');
    }
  ...
  async validateShareAccess(userId: string, resourceType: string, resourceId: string): Promise<boolean> {
  ...
  async getShareByResource(userId: string, resourceType: string, resourceId: string): Promise<Share | undefined> {
    const [share] = await db
      .select()
      .from(shares)
      .where(and(
  ...
  async getResourceShares(ownerId: string, resourceType: string, resourceId: string): Promise<Share[]> {
