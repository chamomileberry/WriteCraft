...
async function doImport() {
  ...
  console.log(
    `📦 Loading ZIP file: ${(zipBuffer.length / 1024).toFixed(2)} KB`,
  );
...
async function pollImportStatus(jobId, cookie) {
  ...
  while (attempts < maxAttempts) {
    ...
    const statusRes = await fetch(
      `http://localhost:5000/api/import/status/${jobId}`,
      {
    ...
    if (job.progress !== lastProgress) {
      console.log(
        `   Progress: ${job.progress}% (${job.processedItems}/${job.totalItems} items)`,
      );
